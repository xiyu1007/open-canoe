...
所有 Python 脚本通过 `__file__` 推导工程根目录，禁止硬编码绝对路径：
...
| 环境变量      | 用途                   | 示例值                                              |
| ------------- | ---------------------- | --------------------------------------------------- |
| `HOME`      | tkinter 需要 HOME 目录 | `C:/Users/GX`                                     |
| `ST_FLASH`  | ST-Link 烧录工具路径   | `assert/stlink-.../bin/st-flash.exe`              |
| `MAKE_PATH` | GNU Make 可执行文件    | `d:/Software/msys64/usr/bin/make`                 |
| `GCC_PATH`  | ARM GCC 工具链目录     | `d:/STM32/.../gcc-arm-none-eabi-10.3-2021.10/bin` |

### 目录结构约定

| 目录            | 内容                                                               |
| --------------- | ------------------------------------------------------------------ |
| `test/`       | 所有测试脚本（单元测试、硬件测试、GUI 测试、App 模拟测试）         |
| `tools/`      | 构建/烧录/部署工具（build.py, deploy.py, deploy.bat, send_cmd.py） |
| `open-canoe/` | App 源码                                                           |
| `firmware/`   | 固件源码                                                           |
...
uv run python ../test/test_ui_controls.py    # 40 项 UI 控件测试
uv run python ../test/test_hardware.py COM7  # 硬件协议测试
uv run python ../test/test_protocol.py       # 协议编解码测试 (无需硬件)
```
